                    ...
                    if (
                        ...
                        for objective_name, adv_key, mask_key, objective_weight in objective_specs:
                            ...
                            if objective_active_tokens <= 0:
                                ...
                                micro_batch_metrics[f"actor/ppo_kl_{objective_name}"] = 0.0
                                micro_batch_metrics[f"actor/logp_abs_diff_{objective_name}"] = 0.0
                                micro_batch_metrics[f"actor/pg_clipfrac_{objective_name}"] = 0.0
                                micro_batch_metrics[f"actor/pg_clipfrac_lower_{objective_name}"] = 0.0
                                continue
                            ...
                            micro_batch_metrics[f"actor/ppo_kl_{objective_name}"] = kl_old_new
                            micro_batch_metrics[f"actor/logp_abs_diff_{objective_name}"] = logp_abs_diff
                            micro_batch_metrics[f"actor/pg_clipfrac_{objective_name}"] = clipfrac
                            micro_batch_metrics[f"actor/pg_clipfrac_lower_{objective_name}"] = clipfrac_lower
                            ...
                            # Preserve rollout-correction metrics for visibility in actor tab.
                            for metric_key, metric_value in objective_pg_metrics.items():
                                if metric_key.startswith("rollout_corr/"):
                                    micro_batch_metrics[f"{metric_key}_{objective_name}"] = metric_value

                        if agg_active_tokens > 0:
                            micro_batch_metrics["actor/ppo_kl"] = agg_kl_old_new / agg_active_tokens
                            micro_batch_metrics["actor/logp_abs_diff"] = agg_logp_abs_diff / agg_active_tokens
                            micro_batch_metrics["actor/pg_clipfrac"] = agg_clipfrac / agg_active_tokens
                            micro_batch_metrics["actor/pg_clipfrac_lower"] = agg_clipfrac_lower / agg_active_tokens
                        else:
                            micro_batch_metrics["actor/ppo_kl"] = 0.0
                            micro_batch_metrics["actor/logp_abs_diff"] = 0.0
